...
class _Msg:
    ...
    def picks_bear_regime(self, last: float, ma200: float, ma_label: str = "MA200") -> str:
        return (
            "🚨 **BEAR REGIME AKTIF — NO PICKS TODAY**\n\n"
            ...
            "⚠️ _Bukan saran investasi — analisis edukasi_"
        )

    # ── Backtest ─────────────────────────────────────────

    def backtest_start(self, ticker: str, months: int) -> str:
        return f"⚙️ Backtesting **{ticker}** ({months} bulan) + Monte Carlo…"

    def backtest_fetching(self, ticker: str) -> str:
        return f"⚙️ Mengambil data historis & menjalankan backtest **{ticker}**…"

    def backtest_ai(self, ticker: str) -> str:
        return f"⚙️ Backtest selesai. Menyusun evaluasi AI untuk **{ticker}**…"
    ...
    def walkforward_start(self, ticker: str, years: int) -> str:
        return f"⚙️ Walk-forward **{ticker}** ({years} tahun)… (~40 detik)"

    def walkforward_running(self, ticker: str) -> str:
        return f"⚙️ Menjalankan walk-forward window demi window untuk **{ticker}**…"
    ...
    def montecarlo_no_trades(self, ticker: str, months: int) -> str:
        return (
            f"⚠️ Tidak ada trade dalam {months} bulan untuk **{ticker}**.\n"
    ...
    def risk_var_fetching(self, ticker: str) -> str:
        return f"⚙️ Mengambil data historis **{ticker}** (1 tahun)…"

    def risk_var_calculating(self, ticker: str, method: str) -> str:
        return f"⚙️ Menghitung VaR **{ticker}** dengan metode `{method}`…"

    def risk_stress_running(self, tickers_str: str) -> str:
        return f"⚙️ Menjalankan 5 skenario stress test untuk {tickers_str}…"

    def risk_corr_fetching(self, n: int) -> str:
        return f"⚙️ Mengambil return historis untuk {n} ticker…"

    def risk_corr_calculating(self) -> str:
        return "⚙️ Menghitung matriks korelasi & skor diversifikasi…"

    def risk_optimize_fetching(self, n: int) -> str:
        return f"⚙️ Mengambil data historis 2 tahun untuk {n} ticker…"

    def risk_optimize_running(self, method: str) -> str:
        return f"⚙️ Menjalankan optimizer `{method}`…"
    ...
    def alert_duplicate(self, ticker: str, condition: str, price: float) -> str:
        return f"⚠️ Alert **{ticker}** {condition} Rp{price:,.0f} sudah ada."
    ...
    def portfolio_reading(self) -> str:
        return "⏳ Membaca CSV & menganalisis portfolio… (20–40 detik)"

    # ── Market ───────────────────────────────────────────

    def market_loading(self) -> str:
        return "⏳ Mengambil data pasar + regime check…"
